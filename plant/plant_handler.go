...
func NewPlantHandler(plantmRepository *PlantmRepository, plantRepository *PlantRepository, cacheRepository *CacheRepository) *PlantHandler {
	return &PlantHandler{
		plantmRepository: plantmRepository,
		plantRepository:  plantRepository,
		cacheRepository:  cacheRepository,
...
func (h *PlantHandler) GetPrice(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		res := &Cost{Ident: i, Price: strconv.Itoa(p * (end - start))} //// calculating perdiod mult by price
		w.WriteHeader(http.StatusOK)
